...
success, errorMessage = exampleBasedInpainting(self)
        ...
        if self._images['source'] is None and self._images['alpha'] is None:
            msg = 'Source and alpha images are both missing'
            return success, msg
        if self._images['source'] is None:
            msg = 'Source image missing'
            return success, msg
        if self._images['alpha'] is None:
            msg = 'Alpha image missing'
            return success, msg
        source_shape = self._images['source'].shape
        alpha_shape = self._images['alpha'].shape
        if source_shape[0] == alpha_shape[0] and source_shape[1] == alpha_shape[1]:
            success = True
        else:
            msg = 'Dimension of source image is different from alpha image'
            return success, msg
